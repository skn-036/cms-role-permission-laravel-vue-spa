...
const actions = {
    ...
    getPermissions({commit}, page) {
        ...
        axios.get('/api/admin/permissions?page=' + page)
        .then(response => {
    ...
    viewPermission({commit}, id) {
        commit('loadingStatus', true)
        axios.get('/api/admin/permissions/' + id)
        .then(response => {
            if(response.status == 200) {
    ...
    createPermission({commit}) {
        commit('loadingStatus', true)
        axios.get('/api/admin/permissions/create')
        .then(response => {
    ...
    editPermission({commit}, id) {
        commit('loadingStatus', true)
        axios.get('/api/admin/permissions/' + id + '/edit')
        .then(response => {
            if(response.status == 200) {
    ...
    updatePermission({commit}, data) {
        axios.put('/api/admin/permissions/' + data.id, {
            data : data.update
        })
        .then(response => {
            if(response.status == 200) {
    ...
    deletePermission({commit}, routeId) {
        axios.delete('/api/admin/permissions/' + routeId)
        .then(response => {
            if(response.status == 200) {
                commit('message', response.data)
            }
