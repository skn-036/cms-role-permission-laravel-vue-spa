<template>
    ...
    <div
        ...
        v-else-if="roles.viewAll_index == false"
    >
    ...
    <section id="all-roles" class="view-table" v-else>
        ...
        <div v-if="retrieveMessage.response_index" class="text-center">
            <h6
                v-for="(message, index) in retrieveMessage.response_message"
                :key="index"
                :class="[
                    retrieveMessage.response_type == 'Error'
                        ? 'text-danger'
                        : 'text-success',
                    'font-20',
                    'my-3',
        ...
        <table class="table table-striped table-bordered table-hover table-sm">
            ...
            <tbody>
                <tr v-for="(role, index) in roles.data" :key="index">
                    ...
                    <td>
                        <ol>
                            <li v-for="(perm, index) in role.permissions">
                                {{ perm.name }}
                            </li>
                        </ol>
                    </td>
                    <td class="d-flex flex-row justify-content-center">
                        <router-link
                            :to="{ name: 'viewRole', params: { id: role.id } }"
